...
exports.retrieveArticles = (query) => {
    ...
    //SQL hurts
    let sqlQuery = format(`SELECT author, title, articles.article_id, topic, created_at, votes, article_img_url, COALESCE(comments.comment_count, 0) AS comment_count FROM articles
    LEFT JOIN (
    ...
    if (Object.keys(query).length !== 0) {
        ...
        for (key in query) {
            if (approvedQueries.includes(key)) {
                conditionalsArray.push(format(`%I = %L`, key, query[key]))
            } else {
